...
def _decode(logit, charset):
    ...
    for o in out:
        text = charset.get_text(o, padding=False, trim=False)
...
def calc_loss(targets, *res_lists):
    total_loss = 0
    ...
    for iteration in tqdm(
        ...
        if (iteration % opt.val_interval == 0):
            ...
            if (current_score >= best_score):
                ...
                torch.save(model.state_dict(), f"./trained_model/{opt.approach}/StrDA_round{round}.pth")
            else:
                score_descent += 1
        ...
        for label_source in labels_source:
            ...
            label_source_index = torch.tensor(charset.get_labels(label_source))
            label_source_length = torch.as_tensor([len(label_source)+ 1], device = device)
            #label_source_index = F.one_hot(label_source_index_, charset.num_classes).float()
        ...
        all_a_res, all_l_res, v_res = preds_source
        loss_source = calc_loss(labels_source_index, v_res, all_l_res, all_a_res)
        ...
        for label_adapting in labels_adapting:
            ...
            label_adapting_index = torch.tensor(charset.get_labels(label_adapting))
        ...
        all_a_res, all_l_res, v_res = preds_adapting
        loss_adapting = calc_loss(labels_adapting_index, v_res, all_l_res, all_a_res)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--grad_clip", type=float, default=20, help="gradient clipping value. default=20"
    )
    """ Data processing """
    parser.add_argument(
