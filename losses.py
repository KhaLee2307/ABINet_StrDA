...
class MultiLosses(nn.Module):
    def __init__(self, one_hot=True, device = "cuda"):
        super().__init__()
        self.ce = SoftCrossEntropyLoss().to(device) if one_hot else torch.nn.CrossEntropyLoss().to(device)
        
        self.bce = torch.nn.BCELoss().to(device)
    ...
    def _ce_loss(self, output, gt_labels, gt_lengths, idx=None, record=True, reproduce = False):
        ...
        if reproduce: 
            gt_index = flat_gt_labels.max(1)[1]
            loss_temp = self.cross_entropy(flat_pt_logits, gt_index.contiguous())
            return loss_temp
        #print(flat_gt_labels.max(1)[1].shape)
        # loss_temp = cri(
        #         flat_pt_logits.view(-1), flat_gt_labels.contiguous().view(-1)
        #     )
        nll = output.get('nll')
        if nll is not None:
    ...
    def forward(self, outputs, *args):
        ...
        if isinstance(outputs, (tuple, list)):
            ...
            losses = [self._ce_loss(o, *args, reproduce = True) for o in outputs if o['loss_weight']> 0.]
            return sum(losses)
        else:
